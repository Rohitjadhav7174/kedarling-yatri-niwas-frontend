...
const BookingForm = () => {
  ...
  const fetchAvailableRooms = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://kedarling-yatri-niwas-backend-kci5gkpi8.vercel.app/api/rooms/available/${roomType}`);
...
const handleSubmitBooking = async () => {
  ...
  try {
    ...
    const response = await axios.post('http://kedarling-yatri-niwas-backend-kci5gkpi8.vercel.app/api/bookings', formData, {
      headers: {
  ...
  const handleCompletePayment = async () => {
    setLoading(true);
    try {
      await axios.put(`http://kedarling-yatri-niwas-backend-kci5gkpi8.vercel.app/api/bookings/${bookingId}/payment`, {
        paymentStatus: 'paid',
